...
target_scale:
  users: small
# timeline_budget celowo nieustalany na życzenie użytkownika; nie przyjmować domyślnego limitu.
created: 2026-09-13
updated: 2026-09-13
checkpoint:
  current_phase: 8
  ...
  gray_areas_resolved:
    - topic: Typ projektu
      decision: Projekt powstaje od zera; użytkownik potwierdził tryb greenfield.
    - topic: Główna persona
      decision: Podopieczny zapisujący serie w aplikacji webowej; trener pozostaje drugą rolą w MVP. Przepływ nie wymaga telefonu.
    - topic: Źródło problemu
      decision: Przewidywanie; hipoteza do sprawdzenia, bez zmierzonego kosztu.
    - topic: Zakres analizy w MVP
      decision: Czytelne porównanie planu z wykonaniem i wskazanie różnic.
    - topic: Sposób logowania
      decision: E-mail i hasło; przygotowane konta i przypisane relacje, bez samodzielnej rejestracji.
    - topic: Wejście bez zalogowania
      decision: Formularz logowania; dane treningu dostępne dopiero po zalogowaniu i sprawdzeniu uprawnień.
    - topic: Budżet czasu
      decision: Użytkownik wycofał limit czasu i polecił nie ustalać go podczas shaping. Zakres MVP nie jest dopasowywany do budżetu czasu.
    - topic: Główny przepływ MVP
      decision: Trener rozpisuje i przypisuje plan z tygodniowym układem jednostek i okresem ważności, aktualizuje parametry w kolejnych tygodniach; podopieczny oznacza rozgrzewkę jako wykonaną lub pominiętą, zapisuje wyniki i rozlicza wykonanie części właściwej, ocenia trudność ćwiczeń części właściwej i całego treningu, następnie kończy trening; trener porównuje plan z wykonaniem i widzi różnice.
    - topic: Dodatkowe kryteria sukcesu
      decision: Podsumowanie treningu przez AI dla podopiecznego jako dodatkowy, mile widziany efekt, który nie decyduje o sukcesie MVP.
    - topic: Uruchamianie podsumowania AI
      decision: Podopieczny wybiera przycisk „Podsumuj trening” dla zakończonego treningu.
    - topic: Wymagane oceny trudności
      decision: Oceny każdego niepominiętego ćwiczenia części właściwej i całego treningu w skali 1–10 są wymagane do zakończenia treningu; komentarz jest opcjonalny.
    - topic: Ważność i tygodniowy układ planu
      decision: Plan obejmuje jednostki treningowe rozpisane na tydzień, także powtarzające się, oraz okres ważności, np. miesiąc. Trener ręcznie aktualizuje ciężary, powtórzenia i RIR/RPE w kolejnych tygodniach. Liczby jednostek i długość okresu podane przez użytkownika są przykładami.
    - topic: Skala pierwszej wersji
      decision: Do 9 osób łącznie, licząc trenerów i podopiecznych.
    - topic: Wpływ większej skali na reguły
      decision: Rozważanie większej skali nie zmienia reguł obecnego MVP. Docelowe zmiany pozostają poza zakresem tej sesji.
  frs_drafted: 15
  quality_check_status: accepted
---

# GymPlanner — shape notes

Źródło pomysłu: `idea-notes.md` (przeczytane w całości).
...
- Oczekiwanie przekazane w `idea-notes.md`:
...
Przegląd zaakceptowany przez użytkownika 2026-09-13. Sesja shaping zakończona; notatki są gotowe do `/10x-prd`.

| Element | Wynik |
| --- | --- |
| Access Control | Obecne: role, relacje, logowanie i widoczność danych. |
| Business Logic | Obecna jedna główna reguła oraz reguły uzupełniające. |
| Project artifacts | Notatki i pełny checkpoint; 15 FR oraz 4 scenariusze. |
| Timeline-cost acknowledgment | Pominięte na wyraźne polecenie użytkownika; nie ustalać limitu ani domyślnego budżetu czasu. |
| Non-Goals | Obecne i zaakceptowane wyłączenia wymaganego MVP. |
| Preserved behavior | Nie dotyczy projektu greenfield. |

Brak luk w elementach objętych przeglądem. Sprawdzenie hipotezy problemów z arkuszami u rzeczywistych użytkowników pozostaje w `Open Questions`; akceptacja notatek nie oznacza potwierdzenia tej hipotezy.
