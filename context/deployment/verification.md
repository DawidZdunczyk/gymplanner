...
## Integracja przed pushem pełnego MVP — 2026-09-14

- Scalono zdalny commit `ae7a617` z lokalnym MVP i potwierdzonym odbiorem S-01 (`02a70b0`), bez force-push i bez konfliktów.
- Po scaleniu Astro sync → lint → Astro check → build: PASS; 62 pliki, 0 błędów/ostrzeżeń/wskazówek. Testy wdrożenia: 14/14 PASS. `smoke:local`: oba tryby PASS; podglądy zatrzymane.
- Użytkownik potwierdził automatyczną publikację po push do `main` i wyraźnie polecił wykonać push. Poprzednie wpisy o `DEPLOY_ENABLED=false` opisują wcześniejszy stan. Zdalnych migracji ról i treningów nie wykonano w tej sesji; push kodu ich nie stosuje.
