...
1. **Sprawdź, czy podano parametry**:
   - Jeśli ścieżka pliku lub odniesienie do zgłoszenia zostało podane jako parametr, pomiń domyślną wiadomość
...
- Zadanie + ramka + badania → minimalna liczba pytań; skupiam się tylko na decyzjach projektowych rozwiązania, które wymagają Twojego wkładu
...
| Artefakty nadrzędne         | NISKI | ŚREDNI | WYSOKI | Co się zmienia w stosunku do wartości bazowej                                                                                                              |
| --------------------------- | ----- | ------ | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Tylko zadanie (bazowe)      | 4–6   | 7–10   | 11–15 | Pełne pytania we wszystkich istotnych kategoriach.                                                                                       |
| Zadanie + badania           | 3–5   | 5–7    | 8–11  | Pomiń pytania, których odpowiedź znajduje się już w dokumencie badawczym. Nie uruchamiaj ponownie podagentów, aby znaleźć to, co już zostało zmapowane w badaniach.            |
...
| Zadanie + ramka + badania   | 1–2   | 3–5    | 5–7   | Pomiń oba. Zadawaj tylko pytania dotyczące [S]projektowania rozwiązania, które naprawdę wymagają wkładu użytkownika.                                                        |

**Zasada**: każdy przekazany artefakt jest źródłem już podjętych decyzji. Czytanie ich liczy się jako słuchanie użytkownika. Nie pytaj użytkownika o to, co już zapisał.

**Gdy ramka jest obecna**, przeczytaj ją W CAŁOŚCI i traktuj jako autorytatywną:
...
- Jeśli ramka **Pewność: NISKA** jest oznaczona, uwzględnij to w "Otwartych Ryzykach i Założeniach" planu i zadaj JEDNO pytanie wyjaśniające, jak postępować (najpierw zweryfikuj, lub planuj z uwzględnieniem ryzyka).
- NIE badaj ponownie ramki. Ramka odpowiada za sformułowanie problemu; Ty odpowiadasz za projekt rozwiązania.

**Gdy badania są obecne**, przeczytaj je W CAŁOŚCI i użyj jako podstawy bazy kodu:
- Sekcja "Odnośniki do kodu" JEST Twoim ugruntowaniem bazy kodu — nie uruchamiaj ponownie agentów Explore, aby znaleźć te same pliki.
- "Wnioski architektoniczne" bezpośrednio zasilają "Analizę Bieżącego Stanu".
...
1. **Natychmiast i W CAŁOŚCI przeczytaj wszystkie wspomniane pliki**:
...
2. **Uruchom początkowe zadania badawcze w celu zebrania kontekstu** (pomiń lub zawęź na podstawie Kroku 1.0):
   ...
   - **Agent ogólnego przeznaczenia** (`subagent_type: "general-purpose"`) — do głębszej analizy, która może wymagać czytania wielu plików i syntezy wyników. Użyj do zrozumienia złożonych systemów.
...
4. **Analizuj i weryfikuj zrozumienie**:
   ...
   - **Przeprowadź test najmniejszego kontrprzykładu przed wyborem pytań do wywiadu i zachowaj wynik jako notatkę roboczą.** Uruchamia się on na słowach rankingowych, selekcyjnych i stanowych, których żądanie używa bez definiowania — "top N", "zwycięzca", "najlepszy", "najnowszy", "pierwszy", "duplikat", "aktywny", "do końca". Dla uporządkowanych selekcji umieść równe wartości porównawcze po obu stronach odcięcia; dla zliczanych zbiorów zmieniaj regułę tożsamości/równoważności; dla progów stanu zmieniaj inkluzywność i zegar sterujący. Dla każdego przypadku zanotuj jedną linijkę dla siebie — termin, kontrprzykład, co użytkownik zobaczyłby inaczej — *zanim* sprawdzisz, co robi kod; następnie przeczytaj implementację i dodaj jej odpowiedź jako kolejną linijkę. Rozstrzygnięcie, które kod wykonuje według id, kolejności wstawiania lub pozycji w tablicy, nie jest decyzją, którą ktoś podjął, więc nigdy nie zamyka notatki. Każda notatka, której wyniki różnią się na ekranie, staje się pytaniem pierwszej rundy; istniejące zachowanie dostarcza jedną opcję, a nie odpowiedź. Notatka jest rusztowaniem do wywiadu, a nie sekcją planu.
...
5. **Przedstaw świadome zrozumienie i oceń złożoność**:
   ...
   - [Opcjonalnie: słowo w żądaniu, które czytam na dwa sposoby — kontrprzykład i to, co kod robi tam dzisiaj — o które zapytam najpierw]
   ```
   ...
   **Ocena złożoności: [WYSOKA / ŚREDNIA / NISKA]**
   ...
   [2-3 zdania wyjaśnienia, DLACZEGO ten poziom złożoności, odwołujące się do konkretnych czynników:
   ...
   decyzje dotyczące [lista kluczowych obszarów decyzyjnych: architektura, przypadki brzegowe, model danych, UX, testowanie itp.].

   Czy to wydaje się słuszne, czy chciałbyś dostosować poziom złożoności?
   ```

   Zapytaj użytkownika: "Czy ta ocena złożoności odpowiada Twoim oczekiwaniom?" z następującymi opcjami:
   - "Zgadzam się — kontynuuj z [N] pytaniami" (opis: "Ocena jest dokładna, zagłębmy się w szczegóły.")
   - "Wyżej — zadaj więcej pytań" (opis: "Jest więcej złożoności niż zidentyfikowano. Wyjaśnię, czego brakuje.")
   ...
   **Skala złożoności:**
   ...
   | ---------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | **NISKI**  | 4-6       | Proste zadanie z jasnymi wymaganiami. Niewiele ruchomych części, zgodne z ustalonymi wzorcami lub konwencjami, ograniczone niewiadome. Przykłady oprogramowania: zmiana pojedynczego pliku, drobna zmiana konfiguracji. Przykłady poza oprogramowaniem: zarys pojedynczego tematu, prosta modyfikacja procesu.                                 |
   ...
   | **WYSOKI** | 11-15     | Zagadnienia przekrojowe, znaczące niewiadome, wielu interesariuszy lub ograniczeń. Wymaga myślenia architektonicznego, ryzyko kosztownych przeróbek w przypadku błędu. Przykłady oprogramowania: przeprojektowanie systemu, migracja danych. Przykłady poza oprogramowaniem: strategia uruchomienia wielokanałowego, gruntowna przebudowa programu nauczania, zmiana procesu organizacyjnego. |
...
6. **Zadawaj głębokie, dociekliwe pytania**:
   ...
   **Zasady strukturyzowania pytań:**
   ...
   - Nagłówek `header` powinien być krótki (max 12 znaków): "Zakres", "Przypadki brzegowe", "Priorytet"
   ...
   **Każda opcja MUSI zawierać sygnał rekomendacji i analizę kompromisów:**
   ...
   - `description` każdej opcji musi mieć następujący format:
     `[1-zdaniowe wyjaśnienie, co to robi] · Siła: [kluczowa zaleta] · Kompromis: [kluczowy koszt lub ryzyko]`
   ...
   **Przykład pytania z rekomendacjami (oprogramowanie):** `Conflicts` to `[S]` — architektura rozwiązania; zawsze zadawane, nawet gdy ramka zdefiniowała problem.
   ...
   Zapytaj użytkownika: "Jak system powinien obsługiwać konflikty, gdy dwóch użytkowników edytuje jednocześnie?" z następującymi opcjami:
   - "Ostatni zapis wygrywa" (opis: "Późniejszy zapis cicho nadpisuje wcześniejszy. · Siła: Zero dodatkowej złożoności, brak potrzeby zmian w interfejsie użytkownika. · Kompromis: Użytkownicy mogą stracić pracę bez ostrzeżenia — akceptowalne tylko, jeśli edycje są rzadkie lub niskiego ryzyka.")
   ...
   - "Oparte na blokadach" (opis: "Pierwszy edytor blokuje zasób; inni widzą tylko do odczytu, dopóki nie zostanie zwolniony. · Siła: Całkowicie zapobiega konfliktom — najprostszy model mentalny dla użytkowników. · Kompromis: Przestarzałe blokady wymagają TTL + logiki czyszczenia; blokuje legalną równoczesną pracę.")
   ...
   **Przykład pytania z rekomendacjami (poza oprogramowaniem — treść/strategia):** `Depth` to `[D]` — diagnostyka dotycząca odbiorców/zakresu; pomiń, jeśli brief ramowy już ustalił, dla kogo to jest.
   ...
   Zapytaj użytkownika: "Jaki poziom szczegółowości technicznej powinien mieć moduł kursu?" z następującymi opcjami:
   - "Przegląd koncepcyjny" (opis: "Zasady wysokiego poziomu, bez kodu. · Siła: Dostępny dla wszystkich poziomów umiejętności, szybszy w produkcji. · Kompromis: Zaawansowani uczniowie mogą uznać go za zbyt płytki — ryzyko utraty zaangażowania.")
   - "⭐ Zalecane: Praktyczne z przykładami z przewodnikiem" (opis: "Koncepcje połączone z ćwiczeniami krok po kroku. · Siła: Równoważy zrozumienie i praktykę — pasuje do formatu, który uzyskał najwyższe wskaźniki ukończenia w 10xDevs2. · Kompromis: 2-3 razy więcej czasu na przygotowanie lekcji; wymaga działających repozytoriów przykładów.")
   - "Głębokie zanurzenie z otwartymi wyzwaniami" (opis: "Minimalne rusztowanie, problemy z prawdziwego świata. · Siła: Wymusza prawdziwe rozwiązywanie problemów, najwyższe zapamiętywanie wiedzy. · Kompromis: Wysokie ryzyko rezygnacji dla mniej doświadczonych uczniów; trudniejsze do wsparcia na dużą skalę.")
   ...
   **Każda kategoria jest oznaczona `[D]` (diagnostyczna — dotycząca problemu) lub `[S]` (rozwiązanie — dotycząca sposobu budowania).** Gdy w Kroku 1.0 dostarczono brief ramowy, **pomiń wszystkie kategorie `[D]`** — ramka je ustaliła. Zawsze zadawaj pytania z kategorii `[S]`, które nadal wymagają wkładu użytkownika.

   **Kategorie uniwersalne (wszystkie dziedziny, wszystkie poziomy):**
   - **Granice zakresu** `[D]`: Co jest w środku, a co poza
   - **Przypadki brzegowe / tryby awarii** `[S]`: Co się dzieje, gdy coś pójdzie nie tak lub stanie się dziwne (obsługa implementacji, nawet jeśli ramka nazwała klasę obserwacji). Zacznij od notatek kontrprzykładów z Kroku 1.1 punkt 4: umieść konkretne dane w pytaniu, a gdy kod już implementuje jedno odczytanie, wymień je jako opcję oznaczoną `(bieżące zachowanie)` — oznacz gwiazdką tylko, jeśli jego wynik jest taki, którego użytkownik broniłby bez wspominania o implementacji
   ...
   **Kategorie strategii / procesu (dodaj w zależności od złożoności):**
   ...
   - **Ramy pomiarowe** `[D]`: Wskaźniki wyprzedzające vs opóźnione, jak korygować kurs
   ...
   **O co NIE pytać:**
   - Wszystko, co zostało już ustalone w artefaktach nadrzędnych (brief ramowy, dokument badawczy) — ponowne pytanie to tryb awarii, któremu ma zapobiegać to skalowanie
...
1. **Badanie wzorców implementacji i wcześniejszych prac**:
   ...
   **Dla zadań poza programistycznych**, zbadaj pliki kontekstowe i wcześniejsze prace:
...
6. **Przedstaw wyniki i opcje projektowe**:
   ...
   **Bieżący stan:**
   ...
   Zapytaj użytkownika: "Które podejście implementacyjne powinniśmy zastosować?" z następującymi opcjami:
...
1. **Przedstaw zarys planu i uzyskaj ustrukturyzowaną opinię**:
   ...
   Następnie zapytaj użytkownika: "Czy ten podział faz wygląda dobrze?" z następującymi opcjami:
   ...
   - "Zbyt ogólne" (opis: "Podziel niektóre fazy — są ukryte złożoności.")
...
1. **Rozwiąż folder zmian, a następnie zapisz plan** do `context/changes/<change-id>/plan.md`.
   ...
   - **Zsynchronizuj mapę drogową** (najlepszy wysiłek): jeśli `context/foundation/roadmap.md` zawiera element, którego `Change ID` jest równe `<change-id>`, zmień status tego elementu na `Status: planning`. Zobacz "## Synchronizacja statusu mapy drogowej" poniżej. Nigdy nie blokuje; większość zmian nie będzie śledzić mapy drogowej.
2. **Użyj tej struktury szablonu** (bloki faz zawierają zwykłe punktorowanie — `- ` zamiast `- [ ]` — a pojedyncza kanoniczna sekcja `## Postęp` na dole jest właścicielem stanu pola wyboru, zobacz `references/progress-format.md` dla umowy):
...
Ta sekcja zawiera **ograniczenia, pułapki i wymagania dotyczące kolejności, które implementator musi znać, zanim dotknie kodu** — fakty, które asystent AI ustala podczas Badań i Odkryć (Krok 2), a które nie są widoczne tylko na podstawie ścieżek plików.

To NIE jest miejsce do wcześniejszego decydowania o implementacji. Domyślnie: **pomijaj** całą sekcję. Dołącz nagłówek poniżej TYLKO wtedy, gdy coś naprawdę zaskakującego lub nośnego ma zastosowanie — i napisz 1-3 zdania, a nie szablony punktorów.

- **Czas i cykl życia** — uwzględnij tylko, jeśli istnieje nieoczywista kolejność, wyścig lub hak cyklu życia, który implementator w przeciwnym razie by przeoczył.
...
- **Debugowanie i obserwowalność** — uwzględnij tylko, gdy istnieje specyficzna metoda weryfikacji lub potrzeba instrumentacji wykraczająca poza standardowe logowanie.
...
Fragment kodu pojawia się tutaj TYLKO wtedy, gdy zmiana jest nieoczywista — trudne wyrażenie regularne, nietypowe wywołanie API, nieintuicyjna kolejność, obejście znanego błędu lub kontrakt sygnatury, od którego zależą inne części planu. W przypadku rutynowych edycji (dodanie pola, podłączenie obsługi, podążanie za istniejącym wzorcem), opisz kontrakt i zakończ. Domyślnie: brak fragmentu.]
...
**Uwaga implementacyjna**: Po zakończeniu tej fazy i wszystkich automatycznych weryfikacji, zatrzymaj się tutaj na ręczne potwierdzenie od człowieka, że testy ręczne zakończyły się sukcesem, zanim przejdziesz do następnej fazy. Bloki faz używają zwykłych punktorów — odpowiadające im pola wyboru `- [ ]` dla tych elementów znajdują się w sekcji `## Postęp` na dole planu.
...
[2-3 zdania: co budujemy/robimy i motywacja. Jeśli brief ramowy był wejściem, umieść tutaj dosłownie Przeformułowane (lub Potwierdzone) Oświadczenie o Problemie — to jest "dlaczego" w najostrzejszej formie.]

## Punkt wyjścia

[1-2 zdania: co istnieje dzisiaj, na czym ten plan się opiera lub co zmienia. Ugruntuj czytelnika w bieżącym stanie, aby zrozumiał różnicę. Jeśli ramka to badała, podsumuj z jej Badania Hipotez, zamiast powtarzać.]

## Pożądany stan końcowy

[2-3 zdania: jak wygląda świat po zakończeniu tego planu. Opisz konkretny, widoczny dla użytkownika wynik — nie metryki, ale doświadczenie lub możliwość, która teraz istnieje.]

## Kluczowe podjęte decyzje

Gdy brief ramowy lub dokument badawczy był wejściem, oznacz kolumnę **Źródło**, aby pokazać, skąd pochodzi decyzja. Pozwala to czytelnikom zobaczyć pochodzenie: co zostało ustalone wcześniej, a co zdecydowano podczas tej sesji planowania.
...
**Poza zakresem:** [Lista punktowana tego, co jest wyraźnie wykluczone]
...
3. **Kluczowe zasady briefu**:
   - Musi mieścić się na około 2 wydrukowanych stronach (~60-80 linii markdown). Jeśli wychodzi dłużej, skróć.
...
4. **Iteruj na podstawie opinii** - bądź gotów do:
...
Zrób to w Kroku 4 (zaraz po oznaczeniu `change.md` jako `planned`). Wyszukiwanie jest **obowiązkowe**; "najlepszy wysiłek" obejmuje tylko *edycje* — brakująca mapa drogowa lub nieznaleziony cel są pomijane cicho i nigdy nie blokują, nie wyświetlają monitów ani nie przerywają działania. Nie pomijaj sprawdzania, zakładając, że nie ma mapy drogowej.
...
2. Przeczytaj plik. Poszukaj `<change-id>` użytego jako `Change ID`:
   ...
   Dopasowanie jest tylko dokładnym ciągiem znaków. **Brak dopasowania** → wydrukuj `ℹ context/foundation/roadmap.md nie zawiera elementu z ID Zmiany "<change-id>" — mapa drogowa pozostaje nietknięta.` i zatrzymaj się tutaj.
3. **Znaleziono dopasowanie** → jeśli `- **Status:**` elementu jest już `planning`, `in-progress` lub `done`, pozostaw go nietkniętym (**tylko do przodu**: nigdy nie cofaj bardziej zaawansowanego statusu) i zatrzymaj się. W przeciwnym razie zastosuj obie edycje za pomocą narzędzia Edit — każda niezależna i najlepszy wysiłek; pomiń pod-edycję, której cel nie znajduje się tam, gdzie umieszcza go szablon `/10x-roadmap`, i zanotuj pominięcie. Dotknij tylko pola `Status`:
...
1. **Bądź sceptyczny**:
   ...
   - Słowo rankingowe lub selekcyjne, którego żądanie używa bez definiowania, pozostaje nierozstrzygnięte, dopóki jego kontrprzykład nie zostanie przedstawiony użytkownikowi (Krok 1.1 punkt 4)
...
6. **OBOWIĄZKOWE: Skalowane złożonością głębokie pytania**:
...
7. **Brak otwartych pytań w ostatecznym planie**:
   ...
   - Kontrprzykład, który użytkownik zdecydował, trafia do już istniejących sekcji — nazwany test lub kryterium sukcesu, gdy zaakceptowany, "Czego NIE robimy", gdy odrzucony. Brak nowej sekcji dla niego
   - Podsekcje "Krytyczne szczegóły implementacji" są opcjonalne: uwzględnij je tylko wtedy, gdy ma zastosowanie rzeczywiste ograniczenie, pułapka lub wymóg kolejności. Domyślnie pomijaj. Plan bez tej sekcji nie jest niekompletny.
...
8. **Opisuj intencje, a nie implementację**:
   ...
   - W przypadku rutynowych edycji — dodawania pola, podłączania obsługi, podążania za istniejącym wzorcem — opisz `**Cel**` w 1-2 zdaniach, nazwij `**Kontrakt**` w jednym i zakończ. Implementator (człowiek lub agent) ustala kod na podstawie ścieżki pliku, otaczającego wzorca i intencji
...
- **Weryfikuj wyniki podzadań** — jeśli nieoczekiwane, twórz zadania uzupełniające i porównuj z rzeczywistym kodem

## Zarządzanie kontekstem

Planowanie może być kontekstowo obciążające ze względu na badania + iteracje. Utrzymuj kontekst efektywny:
...
## Przykład sondowania według typu funkcji
...
Zapytaj użytkownika: "Co użytkownik powinien widzieć podczas ładowania nowych elementów?" z następującymi opcjami:
...
- "Spinner na całą stronę" (opis: "Zastąp zawartość spinnerem. · Siła: Najprostszy w implementacji — jeden komponent, brak problemów z układem. · Kompromis: Blokuje całą interakcję; wydaje się zepsuty przy wolnych połączeniach.")
Zapytaj użytkownika: "Ile elementów powinno to obsługiwać płynnie?" z następującymi opcjami:
- "⭐ Zalecane: Setki" (opis: "Standardowa paginacja z przesunięciem. · Siła: Prosta, dobrze zrozumiała, działa z istniejącymi zapytaniami SQL. · Kompromis: Psuje się po około 5 tys. elementów — akceptowalne, biorąc pod uwagę obecne wolumeny danych.")
...
Zapytaj użytkownika: "Co uczeń powinien być w stanie ZROBIĆ po tym module — nie tylko wiedzieć?" z następującymi opcjami:
...
- "Ukończyć ćwiczenie z przewodnikiem" (opis: "Instruktaż krok po kroku z oczekiwanym wynikiem. · Siła: Niski próg wejścia — wszyscy kończą, buduje pewność siebie. · Kompromis: Może produkować 'zombie tutorialowe', którzy potrafią podążać, ale nie potrafią samodzielnie zastosować.")
- "Zdać test wiedzy" (opis: "Quiz lub przegląd kodu dowodzący zrozumienia koncepcyjnego. · Siła: Szybki do stworzenia, łatwy do oceniania na dużą skalę. · Kompromis: Testuje rozpoznawanie, a nie produkcję — uczeń może rozumieć, ale nie być w stanie wykonać.")
Zapytaj użytkownika: "Jak ten moduł powinien radzić sobie z różnymi poziomami umiejętności w grupie odbiorców?" z następującymi opcjami:
- "Jedna ścieżka, zaawansowana" (opis: "Jedna ścieżka skierowana do doświadczonych programistów. · Siła: Głęboka treść, brak prowadzenia za rękę, szanuje czas ekspertów. · Kompromis: Zraża początkujących — odpadną lub zaleją kanały wsparcia.")
- "⭐ Zalecane: Warstwowa głębia" (opis: "Główna ścieżka, którą wszyscy podążają + opcjonalne sekcje pogłębione. · Siła: Wszyscy otrzymują wartość; zaawansowani uczniowie sami wybierają trudniejszy materiał. · Kompromis: Więcej treści do utrzymania; ryzyko ignorowania sekcji 'opcjonalnych'.")
...
`Bottleneck` to `[D]` — czyste sformułowanie problemu (który problem rozwiązać). To jest dokładnie ten rodzaj pytania, który ramka ma rozstrzygnąć. Z briefem ramowym, pomiń to całkowicie; wiodąca hipoteza jest wąskim gardłem.

Zapytaj użytkownika: "Jakie jest główne wąskie gardło w obecnym potoku newslettera?" z następującymi opcjami:
- "⭐ Zalecane: Kuraacja trwa zbyt długo" (opis: "Znajdowanie i ocenianie linków to wolny krok. · Siła: Bezpośrednio celuje w czas do publikacji — automatyzacja kuraacji daje największe oszczędności czasu na podstawie obecnych czasów potoku. · Kompromis: Automatyczna kuraacja ryzykuje utratę osobistego głosu redakcyjnego, który subskrybenci cenią.")
- "Pisanie komentarzy" (opis: "Linki są gotowe, ale pisanie wokół nich jest wolne. · Siła: Wspomagane przez AI tworzenie może skrócić to o połowę. · Kompromis: Intensywne tworzenie przez AI może sprawić, że newsletter będzie wydawał się generyczny — wymaga starannej kalibracji głosu.")
- "Dystrybucja i harmonogramowanie" (opis: "Treść jest gotowa, ale publikowanie jest ręczne. · Siła: Najłatwiejsze do zautomatyzowania — jasne wejścia i wyjścia. · Kompromis: Najmniejszy wpływ, jeśli kuraacja lub pisanie nadal jest wąskim gardłem.")

**Uwaga**: Pytania koncentrują się na **CO powinno się wydarzyć** (wymagania, zachowanie, wyniki) — NIE **JAK to zaimplementować** (wzorce kodu, konkretne narzędzia). Wybór `⭐ Recommended` jest oparty na badaniach i kontekście — użytkownik zawsze ma ostatnie słowo.